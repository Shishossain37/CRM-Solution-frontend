...
import { appointmentregisterfunc, appointmentgetfunc, appointmentdeletfunc } from "../services/Apis"
...
const Appointment = () => {
    ...
    const userGet = async () => {
        ...
        if (selectedMonth && selectedYear) {
            response = await appointmentgetfunc(search, page, { selectedMonth, selectedYear });
        } else {
            response = await appointmentgetfunc(search, page, {});


        }

        if (response.status === 200) {
            setUserData(response.data.appointmentData);
    ...
    const deleteUser = async (id) => {
        const response = await appointmentdeletfunc(id);
    ...
    const submitUserData = async (e) => {
        ...
        if (name === "") {
            ...
            toast.error("Regarding is Required !")
            ...
            const response = await appointmentregisterfunc({ name, date, time, regard });
                                                    ...
                                                    return (
                                                        <React.Fragment key={index}>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>{element.name}</td>
                                                                <td>{`${new Date(element.date).toLocaleDateString()} ${element.time}`}</td>
                                                                <td>{element.regard}</td>
                                                                ...
                                                                <td>
                                                                    <Dropdown>
                                                                        ...
                                                                        <Dropdown.Menu>
                                                                            <Dropdown.Item >
                                                                                <Link to={`/appointment/${element._id}`} className="text-decoration-none">
